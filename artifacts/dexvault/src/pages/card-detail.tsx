...
function calcEstimatedValue(
  ...
  // For single-variant cards the one variant IS the standard printing.
  // Count all copies (generic + variant-tracked) at that price.
  if (availableVariants.length === 1) {
    const price = availableVariants[0].price ?? 0;
    const variantQty = variantMap[availableVariants[0].key] ?? 0;
    return { total: (genericQty + variantQty) * price, untrackedGeneric: 0 };
  }

  // Multi-variant: sum explicitly tracked copies at each variant's price.
  const variantValue = availableVariants.reduce((sum, v) => {
    return sum + (variantMap[v.key] ?? 0) * (v.price ?? 0);
  }, 0);

  // Generic (untracked) copies can't be priced when multiple variants exist.
  const untrackedGeneric = genericQty > 0 ? genericQty : 0;

  return { total: variantValue, untrackedGeneric };
}
...
export default function CardDetail() {
  ...
  // Progressive image loading: show small immediately, fade large in when ready
  ...
  // A card is "single-variant" when only one printing exists (e.g. a secret rare
  // that only comes as holofoil). That variant IS the standard — no picker needed.
  const isSingleVariant = availableVariants.length === 1;

  // For single-variant, the effective total includes both generic qty and any
  // qty the user may have previously logged under the variant key.
  const effectiveTotalQty = isSingleVariant
    ? genericQty + (variantMap[availableVariants[0]?.key ?? ''] ?? 0)
    : totalQty;
    ...
    <div className="flex flex-col md:flex-row gap-8 pt-4 md:pt-8 animate-in fade-in duration-500">
      ...
      <div className="w-full md:w-1/3 lg:w-1/4 shrink-0">
        <div className="relative w-fit">
          ...
          >
            {/* Small image — visible immediately, likely already cached from grid */}
            <img
              ...
              decoding="async"
            />
            {/* Large image — fades in over the small once downloaded */}
            <img
      ...
      <div className="flex-1 space-y-6">
        ...
        <div>
          ...
          {/* Estimated value — prominent, shown when card is owned */}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
              <div className="flex items-center gap-4 w-full sm:w-auto">
                <div className="flex items-center">
                  ...
                  <span className="w-12 text-center font-bold font-mono text-xl">
                    {effectiveTotalQty}
                  </span>
                  <Button
                    variant="outline" size="icon"
        ...
        <Card className="border-border">
          <CardContent className="p-4">
            ...
            {availableVariants.length === 0 ? (
              ...
              // ── Single-variant card: this IS the standard printing ──────────
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-sm">
                  ...
                  {availableVariants[0].price != null ? (
                    <span className="font-mono font-bold text-green-600 dark:text-green-400">
                      ${availableVariants[0].price.toFixed(2)} market
                    </span>
                  ) : (
                    <span className="text-xs text-muted-foreground">no price data</span>
                  )}
                </div>
                {effectiveTotalQty > 0 && availableVariants[0].price != null && (
                  <p className="text-xs text-muted-foreground">
                    {effectiveTotalQty} {effectiveTotalQty === 1 ? 'copy' : 'copies'} ×{' '}
                    ${availableVariants[0].price.toFixed(2)} ={' '}
              ...
              // ── Multi-variant card: show per-variant tracking ──────────────
              <div className="divide-y divide-border">
                ...
                {/* Legacy variant rows (keys no longer in the pricing data) */}
                {Object.entries(variantMap)
